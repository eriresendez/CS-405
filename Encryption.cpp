...
std::string read_file(const std::string& filename)
{
    // std::string file_text = "John Q. Smith\nThis is my test string";
      ...
      // constructor opening file
    std::ifstream file(filename);
    if (!file.is_open()) {
        // if file doesn't open, error message presented
        std::cerr << "Error opening file: " << filename << std::endl;
        return "";
    }
    // implementing loading the file into a string
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    if (!file.is_open()) {
        // if file fails to open, error message with file name
        std::cerr << "Error opening file for writing: " << filename << std::endl;
        return;
    }

    // current date for timestamp in year, month, day format
    char timestampBuffer[100] = { 0 };
    struct tm ltm;
    std::time_t timestamp = std::time(0);
    localtime_s(&ltm, &timestamp);
    std::strftime(timestampBuffer, sizeof(timestampBuffer), "%Y-%m-%d", &ltm);

    // writing data in specified format 
    // Line 1: student name
...
int main()
{
    ...
    const std::string file_name = "inputdatafile2.txt";
