...
#include <stdexcept>    // std:: overflow_error and std::underflow_error
...
/// <returns>start + (inrement *steps)</returns>
template <typename T>
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (decrement > 0 && result < std::numeric_limits<T>::min() + decrement) {
            ...
            throw std::underflow_error("Underflow detected in subtraction");
        }
        result -= decrement;
    }
...
void test_overflow()
{
    ...
    try {
        ...
        std::cout << "Overlfow detected: " << e.what() << std::endl;
    }
}
